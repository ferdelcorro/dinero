...
(function($){
	$.fn.extend({ 
		lightSwitch: function(name,imagen, options)
		{
			var defaults = 
			{
				animSpeed : 120,
				hoverSpeed : 100,
				switchImg : '/img/'+imagen+'.png',
				switchImgCover: '/img/switchplate.png',
				switchImgCoverWidth : '39px',
				switchImgCoverHeight : '21px',
				disabledImg : '/img/disabled.png',
				onShift : '2px 0px',
				offShift : '-18px 0px',
				peekOff : '-3px 0px',
				peekOn : '-15px 0px'
    		...
    		return this.each(function() {
				...
				{			
					$(this).css({'display':'none'}).after('<span class="'+name+'"><img src="'+o.switchImgCover+'" width="'+o.switchImgCoverWidth+'" height="'+o.switchImgCoverHeight+'" /></span>'); //'display':'none'
				}
				$(this).next('span.'+name).css({'display':'inline-block','background-image':'url("'+o.switchImg+'")','background-repeat':'no-repeat','overflow':'hidden','cursor':'pointer','margin-right':'2px'});

				$(this).next('span.'+name).click(function(){
				...
				$(this).next('span.'+name).css({'background-position': $(this).is(':checked') ? o.onShift : o.offShift }); // setup default states
